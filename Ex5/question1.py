...
def bubblesort(seq, reverse=False):
    ...
    for idx in range(seqlen - 1):  # outer for loop for iterating through the elements
        for j in range(seqlen - idx - 1):  # inner for loop for number of passes for each iteration
    ...
    runtimedata = {"comparisons": comparisoncount, "swappings": swappingcount,
...
def selectionsort(seq, reverse=False):
    ...
    for idx in range(seqlen):  # iterating through the list
        min_idx = idx  # setting the minimum index(element) as the first index(element)
        for j in range(idx + 1, seqlen):  # eliminating the previous index element as it is already sorted
            if reverse:  # if reverse is True, then sort in ascending
                ...
                if seq[j] < seq[min_idx]:
                    min_idx = j
        swappingcount += 1
        seq[idx], seq[min_idx] = seq[min_idx], seq[idx]  # Swapping current element and the relative first element
