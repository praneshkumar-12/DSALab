...
class Stack:
    ...
    def resize(self, size):
        ...
        for i in range(self.top):
            temp[i] = self.stack[i]  # Copy elements from the old array to the new array
        self.stack = temp  # Replace the old array with the new array
    ...
    def push(self, elt):
        ...
        if self.top == self.cap:
            self.resize(self.cap * 2)  # Double the capacity if the stack is full
